...
//  printf("Creado hilo %i con prioridad %i\n", t_state[i].tid, t_state[i].priority);
  ...
  {
    /*si el proceso que se esta ejecutando es uno
...
void timer_interrupt(int sig)
{
  ...
  {
  //para este caso, se llamara a nuestra nueva funcion antiinanicion en cada tick de reloj
  antiinanicion();
    return;
  }
...
/*esta funcion empezara a disminuir el atributo hungry de cada proceso de baja prioridad para
que no haya ningun estado de inanicion. Cuando el valor hungry de un proceso llegue a 0, se reseteara y se pasara a la cola de alta prioridad.
Usaremos una cola auxiliar para recibir a ir cogiendo todos los procesos de la cola de baja prioridad y disminuir su hungry*/
void antiinanicion(){

 if (queue_empty(FIFOcola) == 0 || running->priority == 1)
  {
    hungry = hungry +1;
   // printf("%ld\n", hungry);
  }

  //si la variable hungry ya ha llegado al valor de starvation, meteremos todos los procesos de la cola de baja prioridad en la cola de alta prioridad. 
  if (hungry == STARVATION)
  {
    hungry = 0L;
    while(queue_empty(RRcola)==0){
       disable_interrupt();
       TCB * procaux = dequeue(RRcola);
       printf("*** THREAD %i PROMOTED TO HIGH PRIORITY QUEUE\n", procaux->tid );
       enqueue(FIFOcola, procaux);
       enable_interrupt();
...
void activator(TCB* next){
  ...
  //reseteamos la variable hungry ya que va a entrar a la cola de nuevo
  hungry = STARVATION;
